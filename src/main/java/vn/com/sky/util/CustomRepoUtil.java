...
public class CustomRepoUtil extends BaseR2dbcRepository {
    ...
    public Mono<Boolean> isTextValueExisted(String tableName, String columnName, String value) {
        ...
        var ret = this.databaseClient().execute(genSql(methodName, "tableName", "columnName", "value")).bind("tableName", tableName).bind("columnName", columnName);
    ...
    public Mono<Boolean> isTextValueDuplicated(String tableName, String columnName, String value, Long id) {
        ...
        var ret = this.databaseClient().execute(genSql(methodName, "tableName", "columnName", "value", "id")).bind("tableName", tableName).bind("columnName", columnName).bind("id", id);
