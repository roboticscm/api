...
public interface RoleDetailRepo extends ReactiveCrudRepository<RoleDetail, Long> {
    @Query("select * from role_detail where deleted_by is null and role_id=:roleId and menu_org_id in (select id from menu_org where org_id=:departmentId and menu_id=:menuId)")
